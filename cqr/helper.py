...
def run_icp(nc, X_train, y_train, X_test, idx_train, idx_cal, significance, condition=None,train=False):
    ...
    if train == True:
        predictions_train = icp.predict(X_train, significance=significance)

        y_lower_train = predictions_train[:, 0]
        y_upper_train = predictions_train[:, 1]

        return y_lower, y_upper, y_lower_train, y_upper_train
    else:
